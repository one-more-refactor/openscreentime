...
pub struct HeartbeatReq {
    ...
    #[serde(default)]
    #[allow(dead_code)] // accepted from agents; not yet persisted server-side
...
async fn handle_ws(st: AppState, agent: AgentAuth, socket: WebSocket) {
    ...
    if let Ok(cmds) = pull_pending_commands(&st.db, device_id).await {
        for c in cmds {
            let _ = st
                .hub
                .push(device_id, json!({ "kind": "command", "command": c }))
                .await;
        }
    }
...
/// Handle one inbound WS frame from an agent.
///
/// Frame kinds:
///   - `{ kind:"event", type, severity?, device_user?, payload }`
///   - `{ kind:"ack",   command_id, status, result? }`
///   - `{ kind:"ssh_data", session_id, data }` (reverse-SSH bytes; `data` is a
///     UTF-8/opaque string in the skeleton — production frames base64 bytes)
///   - `{ kind:"heartbeat" }`
async fn handle_ws_frame(st: &AppState, agent: AgentAuth, v: Value) {
    match v.get("kind").and_then(|k| k.as_str()) {
        ...
        Some("ack") => {
            if let Some(cmd_id) = v
                .get("command_id")
                .and_then(|c| c.as_str())
                .and_then(|s| Uuid::parse_str(s).ok())
            {
                let status = v.get("status").and_then(|s| s.as_str()).unwrap_or("acked");
        ...
        Some("ssh_data") => {
                ...
                .and_then(|s| Uuid::parse_str(s).ok())
            {
                let data = v
                    ...
                    .unwrap_or_default();
                st.hub.ssh_data_from_agent(session_id, data).await;
            }
        }
        Some("heartbeat") => {
